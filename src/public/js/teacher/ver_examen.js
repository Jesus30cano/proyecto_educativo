...
function loadExamData(data) {
  ...
  document.getElementById("competence-name").textContent =
    data.data.competenceName;
...
function loadQuestions(questions) {
  ...
  questions.forEach((question, index) => {
    ...
    question.options.forEach((option, optIndex) => {
      ...
      optionsHTML += `
        <div class="option">
          <input type="radio" id="q${question.id}_${letra}" name="question_${
        question.id
      }" value="${letra}">
          <label for="q${question.id}_${letra}">${letra.toUpperCase()}. ${
        option.text
      }</label>
        </div>
      `;
