...
async function mostarDatosPersonales() {
  try {
    ...
    if (data.status !== "success") {
      console.error("❌ Error en la respuesta del servidor1:", data.message);
      return;
    }
...
async function mostarDatosEmergencia() {
  try {
    ...
    const data = await response.json();
    if (data.status !== "success") {
      console.error("❌ Error en la respuesta del servidor2:", data.message);
      return;
    }
...
function mostarDatosEmergencia2(data) {
  ...
  document.getElementById("emergencia_parentesco").textContent =
    data.parentesco;
  ...
  document.getElementById("emergencia_observaciones").textContent =
    data.observaciones;
}
