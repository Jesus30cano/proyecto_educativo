...
function renderizarCompetencias(calificaciones) {
    ...
    if (Object.keys(porCompetencia).length === 0) {
        contenedor.innerHTML = '<p class="text-muted mb-0">No hay competencias registradas.</p>';
    }
...
function renderizarTabla(calificaciones) {
    ...
    calificaciones.forEach(item => {
        ...
        const fechaEntrega = item.fecha_entrega ? formatearFecha(item.fecha_entrega) : 'N/D';
